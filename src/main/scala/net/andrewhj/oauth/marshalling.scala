...
trait Marshalling extends DefaultJsonProtocol with SprayJsonSupport with MetaMarshallers {
  ...
  implicit def eitherFailureStatusCodeMarshaller[A, B](code: A ⇒ StatusCode)(implicit ma: Marshaller[A], mb: Marshaller[B]): ToResponseMarshaller[Either[A, B]] =
    Marshaller[Either[A, B]] { (value, ctx) ⇒
      value match {
        case Left(a) ⇒
          val mc = new CollectingMarshallingContext()
          ma(a, mc)
          ctx.handleError(ErrorResponseException(code(a), mc.entity))
        case Right(b) ⇒
          (200, mb(b, ctx))
      }
    }

  implicit def eitherStatusCodeMarshaller[A, B](code: Either[A, B] ⇒ StatusCode)(implicit ma: Marshaller[A], mb: Marshaller[B]): ToResponseMarshaller[Either[A, B]] =
    Marshaller[Either[A, B]] { (value, ctx) ⇒
      value match {
        case l @ Left(a) ⇒
          val mc = new CollectingMarshallingContext()
          ma(a, mc)
          ctx.handleError(ErrorResponseException(code(l), mc.entity))
        case r @ Right(b) ⇒
          (code(r), mb(b, ctx))
      }
    }
